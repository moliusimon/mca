...
def load_mnist(scale=True):
    mnist_data = loadmat('/mnt/Storage/Datasets/domain_adaptation/svhn_mnist/mnist_data.mat')
    if scale:
        ...
        mnist_labels_train = np.cast[np.int64](mnist_data['label_train'])
        mnist_labels_test = np.cast[np.int64](mnist_data['label_test'])
    else:
